...
export default function Suppliers() {
  ...
  const filtered = suppliers.filter((s) => 
    (s.name + " " + s.company + " " + s.phone).toLowerCase().includes(query.toLowerCase())
  );

  const openFormModal = (sup: Supplier | null = null) => {
    if (sup) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    let done = false;
    if (editingSupplier) {
      done = await updateSupplier(editingSupplier.id, payload);
    } else {
      done = await addSupplier(payload);
    }
  ...
  const handleCSVExport = () => {
    ...
    const title = isAr ? "قائمة بيانات الموردين وجهات الاتصال" : "Suppliers Directory & Contact Information";
    const filename = isAr 
      ? `بيانات_الموردين_${new Date().toISOString().split("T")[0]}`
      : `Suppliers_${new Date().toISOString().split("T")[0]}`;

    const headersList = isAr
      ? ["اسم المورد", "الشركة / المؤسسة", "رقم الهاتف", "البريد الإلكتروني", "العنوان"]
      : ["Supplier Name", "Company Name", "Phone", "Email Address", "Address"];
      ...
      <div className="glass-bg neumorphic-flat p-4 rounded-3xl border flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="relative w-full md:max-w-xs">
          ...
          <input
            id="sup-search"
            type="text"
            placeholder={t("supplier_mgmt").split(" ")[1] + " ..."}
            value={query}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.length === 0 ? (
          ...
          filtered.map((s) => (
            ...
            >
              {/* Header */}
              <div className="flex items-start justify-between border-b pb-3 mb-3 text-right">
                <div>
                ...
                {/* Company Label */}
                <span className="px-2.5 py-0.5 rounded-xl text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase">
                  {s.company}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t text-slate-400 select-none">
                <span className="text-[10px] text-slate-400 flex items-center gap-1">
                  ...
                  {language === "ar" ? "مخزن معتمد" : "Verified Vendor"}
                </span>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="صالح البراك (مندوب مبيعات)"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="المدينة الصناعية الثانية، الرياض"
                    className="w-full text-xs font-semibold px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-right"
                  />
                </div>
                ...
                <div className="pt-4 flex gap-3 justify-end items-center">
                  ...
                  >
                    {editingSupplier ? t("save_changes") : t("add_btn")}
                  </button>
                </div>
              </form>
            </motion.div>
